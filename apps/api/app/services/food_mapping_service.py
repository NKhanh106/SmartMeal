...
async def match_multiple_foods(
    ...
    """
    Match multiple food names in batch.
    More efficient than calling match_food_name() in a loop.
    """
    if not food_names:
        return []

    normalized_names = [normalize_vietnamese(name) for name in food_names]

    # Build a combined search pattern for all names
    patterns = [f"%{norm}%" for norm in normalized_names]

    # Fetch all potential candidates in one query
    conditions = []
    for pattern in patterns:
        conditions.append(FoodNutrition.food_name.ilike(pattern))
        conditions.append(FoodNutrition.food_name_vi.ilike(pattern))
        conditions.append(FoodNutrition.food_name_en.ilike(pattern))
    ...
    # Group candidates by which search term they match
    results: list[FoodMatchResult] = []
    for food_name in food_names:
        search_norm = normalize_vietnamese(food_name)

        # Find candidates that match this search term
        matching = [
            c for c in all_candidates
            if search_norm in normalize_vietnamese(c.food_name or "")
            or search_norm in normalize_vietnamese(c.food_name_vi or "")
            or search_norm in normalize_vietnamese(c.food_name_en or "")
        ]

        if matching:
            scored = score_candidates(matching, food_name)
            top = scored[0]
            results.append(FoodMatchResult(
                search_name=food_name,
                matched_food=top.food,
                matched_food_id=top.food.id,
                match_score=top.score,
                match_status=classify_match(top.score)[0],
                alternatives=[
                    FoodMatchCandidate(food=c.food, score=c.score, method=c.method)
                    for c in scored[1:5]
                    if c.score >= PARTIAL_MATCH_SCORE
            ...
            # Fallback: fuzzy across all fetched candidates
            scored = score_candidates(all_candidates, food_name)
            if scored and scored[0].score >= PARTIAL_MATCH_SCORE:
                top = scored[0]
                results.append(FoodMatchResult(
                    search_name=food_name,
                    matched_food=top.food,
